...
def main_test(data = None):
    ...
    from_currency_label = Label(
        bottom_frame, text='Какая валюта:', font=('Poppins 10 bold'), justify=LEFT)
    from_currency_label.place(x=5, y=10)

    to_currency_label = Label(bottom_frame, text='В какую:', font=(
        'Poppins 10 bold'), justify=RIGHT)
    to_currency_label.place(x=160, y=10)

    from_currency_combo = ttk.Combobox(bottom_frame, values=pd.DataFrame(data)[
                                       0].tolist(), width=14, font=('Poppins 10 bold'))
    from_currency_combo.place(x=5, y=30)

    to_currency_combo = ttk.Combobox(bottom_frame, values=pd.DataFrame(data)[
                                     0].tolist(), width=14, font=('Poppins 10 bold'))
    to_currency_combo.place(x=160, y=30)

    amount_label = Label(
        bottom_frame, text='Количество валюты:', font=('Poppins 10 bold'))
